...
static void frudp_spdp_rx_data(frudp_receiver_state_t *rcvr, const frudp_submsg_t *submsg, const uint16_t scheme, const uint8_t *data)
{
  ...
  while ((uint8_t *)item < submsg->contents + submsg->header.len)
  {
    ...
    */
    ...
    {
      part->vid = FreeRTOS_htons(*((frudp_vid_t *)pval));
    }
...
*/
  ...
  scheme->scheme = FreeRTOS_htons(FRUDP_SCHEME_PL_CDR_LE);
  scheme->options = 0;
  /////////////////////////////////////////////////////////////
  ...
  memset(loc->addr.udp4.zeros, 0, 12);
  loc->addr.udp4.addr = FreeRTOS_htonl(FRUDP_DEFAULT_MCAST_GROUP);
  /////////////////////////////////////////////////////////////
  PLIST_ADVANCE(param_list);
  ...
  */
  ...
  if( !frudp_tx( FreeRTOS_htonl( FRUDP_DEFAULT_MCAST_GROUP ), frudp_mcast_builtin_port(), frudp_mcast_builtin_port(), ( const uint8_t * )msg, payload_len ) )
  {
#ifdef PRINT_DEBUG
