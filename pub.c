                              ...
                              const uint32_t num_data_submsgs)
{
  ...
  {
    //woah there partner. don't have space for more pubs
...
void frudp_send_sedp_msgs(frudp_part_t *part)
{
  ...
  if (g_sedp_pub_pub->next_submsg_idx)
  {
    ...
    for (int i = 0; i < g_sedp_pub_pub->next_submsg_idx; i++)
    {
      ...
      printf("catchup SEDP msg %d addressed to reader EID 0x%08x\r\n",
          i, FreeRTOS_htonl((unsigned)pub_submsg->reader_id.u));
    }
    ...
    frudp_tx( dst_addr, dst_port, dst_port, ( const uint8_t * )msg, payload_len );
  }
  else
  ...
  if (g_sedp_sub_pub->next_submsg_idx)
  {
    ...
    for (int i = 0; i < g_sedp_sub_pub->next_submsg_idx; i++)
    {
      ...
      printf("catchup SEDP msg %d addressed to reader EID 0x%08x\r\n",
          i, FreeRTOS_htonl((unsigned)sub_submsg->reader_id.u));
    }
...
*/
  ...
  */
  ...
  int payload_len = &msg->submsgs[submsg_wpos] - ((uint8_t *)msg);
  frudp_tx( FreeRTOS_htonl( FRUDP_DEFAULT_MCAST_GROUP ), frudp_mcast_builtin_port(), frudp_mcast_builtin_port(), ( const uint8_t * )msg, payload_len );
  pub->next_submsg_idx++;
...
void frudp_add_writer(const frudp_writer_t *writer)
{
  ...
  printf("add_writer(%08x => ", 
         (unsigned)FreeRTOS_htonl(writer->writer_eid.u));
  ...
  */
  ...
  for (int i = 0; i < g_frudp_num_writers; i++)
  {
    ...
    if (w->writer_eid.u == pub->writer_eid.u)
    {
      ...
      frudp_tx( part->default_unicast_locator.addr.udp4.addr, part->default_unicast_locator.port, part->default_unicast_locator.port, ( const uint8_t * )msg, udp_payload_len );
    }
  }
  return true;
}
    ...
    const uint8_t *payload, const uint32_t payload_len)
{
  ...
  if (pub->reliable)
  {
    ...
    //user reliable publishing not quite done yet
    return false;
  }
  // craft a tx packet and stuff it
  frudp_msg_t *msg = frudp_init_msg((frudp_msg_t *)g_pub_user_tx_buf);
  ...
  frudp_encapsulation_scheme_t *scheme =
    (frudp_encapsulation_scheme_t *)((uint8_t *)d->data);
  scheme->scheme = FreeRTOS_htons(FRUDP_SCHEME_CDR_LE);
  scheme->options = 0;
