...
static const xMACAddress_t xMulticastMACAddress = { { 0x01, 0x00, 0x5E, 0x7F, 0x00, 0x01 } };
...
uint32_t ulAddressToLookup;
	...
	//alterei
	else if( *pulIPAddress == ipMULTICAST_IP_ADDRESS )
	{
		/* This is a multicast so uses the multicast MAC address. */
		memcpy( ( void * ) pxMACAddress, &xBroadcastMACAddress, sizeof( xMACAddress_t ) );
		eReturn = eARPCacheHit;
	}
...
const xEthernetHeader_t *pxEthernetHeader;
	...
	//alterei
	else if( pxEthernetHeader->xDestinationAddress.ucBytes[ 2 ] == 0x5E )
	{
		/* The packet was to this node directly - process it. */
		eReturn = eProcessBuffer;
	}
...
BaseType_t xChecksumIsCorrect;
	...
	//alterei
	if( ( pxIPHeader->ulDestinationIPAddress == *ipLOCAL_IP_ADDRESS_POINTER ) || ( pxIPHeader->ulDestinationIPAddress == ipBROADCAST_IP_ADDRESS ) || ( *ipLOCAL_IP_ADDRESS_POINTER == 0 ) || ( pxIPHeader->ulDestinationIPAddress == ipMULTICAST_IP_ADDRESS ) )
	{
